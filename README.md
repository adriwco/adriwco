### Olá! Eu sou o <a href="https://www.linkedin.com/in/adriwco">@adriwco</a> <a href="https://github.com/adriwco"><img src="https://user-images.githubusercontent.com/29931326/125177555-2e78db00-e1b3-11eb-9e49-409c4f649cf5.gif" width="30px"></a> <a href="https://github.com/adriwco"><img href="https://github.com/adriwco" src="https://komarev.com/ghpvc/?username=adriwco&color=blueviolet" alt="Profile views"/></a>

<a href="https://github.com/adriwco"><img src="https://raw.githubusercontent.com/khoa083/khoa/main/Khoa_ne/img/Rainbow.gif" width="100%"/></a>

#### Sou um **Desenvolvedor Front-end** com noções de Back-end, foco em **React**, **Node.js** e **PostgreSQL**. Tenho experiência no desenvolvimento de SaaS financeiro, integração de APIs e criação de interfaces responsivas e acessíveis.

- 🔭 Atualmente focado em **React**, **React Native**, **Node.js** e **Arquitetura de Software**.
- 💼 Bacharel em **Sistemas de Informação** e cursando Pós-Graduação em **Desenvolvimento Full Stack**.
- 🚀 Buscando contribuir com soluções escaláveis e de alta performance.

---

Aqui estão as tecnologias e ferramentas que utilizo no meu dia a dia:

| Categoria | Tecnologias |
| :--- | :--- |
| **Front-end** | JavaScript (ES6+), TypeScript, React, React Native, HTML5, CSS3, Tailwind CSS, Context API, Axios, Fetch API, ApexCharts, Leaflet, MVVM Module |
| **Back-end** | Node.js, Express, Prisma ORM, PostgreSQL, APIs RESTful, MQTT |
| **DevOps & Tools** | Git, GitHub, Postman, Docker |
| **Design & Admin** | Figma, Trello, Notion, Google Workspace, Office 365 (Excel, Word, PowerPoint) |

<br>
<br>

<p align="center"> 
<a alt="Gmail" href="mailto:adriwco@gmail.com"><img src="https://user-images.githubusercontent.com/80191040/185791246-a96e7322-3f00-4edf-9ffa-47d7bd002103.svg" width="48px"></a>
<a alt="LinkedIn" href="https://www.linkedin.com/in/adriwco"><img src="https://user-images.githubusercontent.com/80191040/185790979-66a36459-a9b6-4ed1-b9eb-94f219f290f1.svg" width="48px"></a>
<a alt="WhatsApp" href="https://wa.me/5585991983369"><img src="https://user-images.githubusercontent.com/80191040/185155424-a5d28c8e-0c0a-4168-8931-fc597f64fe8a.svg" width="48px"></a>
</p>

<a href="https://github.com/adriwco"><img width=100% title="Final" alt="" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=130&section=footer&text=🧩&fontSize=42&fontColor=fff&animation=twinkling&fontAlignY=75"/></a>
